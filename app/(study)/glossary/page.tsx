...
const grouped = (trainingData as any).glossary.reduce((acc: any, term: any) => {
  const letter = term.term[0].toUpperCase();
  if (!acc[letter]) acc[letter] = [];
  acc[letter].push(term);
  return acc;
}, {} as Record<string, Array<{ id: string; term: string; definition: string }>>);

const letters: string[] = Object.keys(grouped).sort();
...
export default function GlossaryPage() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) =>
        entries.forEach((e) => {
          if (e.isIntersecting)
            setActiveL(e.target.id.replace("glossary-", ""));
        }),
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      {/* Mobile alpha nav */}
      <div className="flex md:hidden overflow-x-auto gap-1 pb-2 mb-4 sticky top-12 bg-gray-950 z-10">
        {letters.map((l) => (
          <button
            key={l}
            onClick={() =>
              document
                .getElementById(`glossary-${l}`)
          ...
          {letters.map((l) => (
            <div key={l} id={`glossary-${l}`}>
              ...
              <div className="space-y-3">
                {grouped[l].map(
                  (entry: { id: string; term: string; definition: string }) => (
                    <div key={entry.id} className="border-b border-gray-800 pb-3">
